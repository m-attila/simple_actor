...
/// Base engine of an actor server
    ...
    R: 'static + Send + Debug,
{
    ...
    pub(crate) fn new(
        ...
        mut state_handler: Box<dyn StateHandler>,
        ...
        let handle = tokio::spawn(async move {
            let exit_val = match state_handler.init(i_name.clone()) {
                Ok(_) => {
                    Self::looping(svr_handler, receiver, internal_sender, i_name.clone()).await
                }
                Err(e) => {
    ...
    /// Message or request processing loop
    async fn looping(
        mut svr_handler: impl ActorServerHandler<Message = ME, Request = MR, Reply = R>,
        mut receiver: mpsc::Receiver<Command<ME, MR, R>>,
        sender: mpsc::Sender<Command<ME, MR, R>>,
        name: String,
    ) -> Res<()> {
        debug!("`{}` actor's receiver loop has been started", name);
        let res = loop {
            match receiver.recv().await {
                None => {
                    break {
                        error!("`{}` actor's receiver channel has been closed", name);
                        Err(SimpleActorError::Receive.into())
                    }
                }
                Some(Command::Stop) => {
                    break {
                        debug!("`{}` actor: stop command has received", name);
                        Ok(())
                    }
                }
                Some(cmd) => {
                    trace!("`{:?}` command has received by `{}` actor", cmd, name);
                    match svr_handler.process(cmd).await {
                        // Synchronous request was processed, reply was sent
                        Ok(None) => (),
                        // Asynchronous request starts with a long heavy computation. Loop have to wait for other task's result
                        Ok(Some(handle)) => {
                            ...
                            tokio::task::spawn(async move {
                                trace!("Heavy computation has started by actor `{}`", i_name);
                                match handle.await {
                                    // heavy task returns a transformed request, which contains the computing result.
                                    // It will be processed by a synchronous request in the request handler.
                                    // This method allows that the state to be modifiable by this result.
                                    Ok(cmd) => {
                                        trace!("Heavy computation was ready in actor `{}`", i_name);
                                        if let Some(command) = cmd {
                                            // Heavy computation was returned with a new request
                                            match command {
                                                Command::Request(_, _) => {
                                                    // The transformed result will be sent to the actor server itself.
                                                    if sender_c.send(command).await.is_err() {
                                                        error!("`{}` actor is unable to send the transformed request to itself", i_name);
        ...
        debug!(
            "`{}` actor's receiver loop was exited with result: `{:?}`",
            name, res
        );
        res
    }
...
mod tests {
    ...
    fn message_actor_test() {
        ...
        rt.block_on(async {
            ...
            let test_state_handler =
                TestStateHandler::new(init_called.clone(), Ok(()), terminate_result);

            // Create test actor server handler
            let test_svr_handler = TestServerHandler {
                process_error: Ok(()),
            };
            // Create actor server
            ...
            // Send message
            sender
                .send(Command::Message("hello".to_string()))
                .await
                .unwrap();
        ...
        rt.block_on(async {
            ...
            // Create test actor server handler
            let test_svr_handler = TestServerHandler {
                process_error: Err(CustomError::ProcessError),
            };
