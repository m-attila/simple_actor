...
/// Reusable actor builder. The build process can be execute more than once.
pub struct ReusableActorBuilder {
    /// Generic builder
    builder: ActorBuilder,
    /// Optional state handler generator that can be set by builder
...
impl ReusableActorBuilder {
    ...
    /// Set state handler's custom implementation
    pub fn state_handler(
        mut self,
        state_handler: impl Fn() -> Box<dyn StateHandler> + 'static,
    ) -> Self {
    ...
    pub fn message_actor<ME>(
        self,
        msg_handler: impl Fn() -> Box<dyn MessageHandler<Message = ME>> + 'static,
    ) -> MessageActorBuilder<ME>
    where
    ...
    pub fn request_actor<MR, R>(
        self,
        req_handler: impl Fn() -> Box<dyn RequestHandler<Request = MR, Reply = R>> + 'static,
    ) -> RequestActorBuilder<MR, R>
    where
    ...
    pub fn hybrid_actor<ME, MR, R>(
        self,
        handler: impl Fn() -> Box<dyn HybridHandler<Message = ME, Request = MR, Reply = R>> + 'static,
    ) -> HybridActorBuilder<ME, MR, R>
    where
        ME: 'static + Send + Debug,
        MR: 'static + Send + Debug,
        R: 'static + Send + Debug,
    ...
    ME: 'static + Send + Debug,
{
    ...
    fn new(
        builder: ReusableActorBuilder,
        handler: impl Fn() -> Box<dyn MessageHandler<Message = ME>> + 'static,
    ) -> Self {
        Self {
            builder,
    ...
    /// Start a new message actor
    pub fn build(&self) -> MessageActor<ME> {
        MessageActor::new(
            self.builder.builder.name.clone(),
            (self.handler)(),
            self.builder
                .state_handler
                .as_ref()
    ...
    R: 'static + Send + Debug,
{
    ...
    fn new(
        builder: ReusableActorBuilder,
        handler: impl Fn() -> Box<dyn RequestHandler<Request = MR, Reply = R>> + 'static,
    ) -> Self {
        Self {
            builder,
            handler: Box::new(handler),
    ...
    R: 'static + Send + Debug,
{
    ...
    /// Build and start a hybrid actor
