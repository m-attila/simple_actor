...
pub(crate) struct ActorHybridServerHandler<ME: Send, MR: Send, R: Send>(
...
where
    ...
    R: Send,
{
    /// Wrap `HybridHandler` into this implementation.
    pub(crate) fn new(
    ...
    R: Send + 'static,
{
    ...
    async fn process(
        &mut self,
        command: Command<Self::Message, Self::Request, Self::Reply>,
    ) -> ProcessResult<Self::Message, Self::Request, Self::Reply> {
        match command {
            Command::Message(message) => {
                ProcessResultBuilder::message_processed(self.0.process_message(message).await)
                    .result()
            }
            command => {
