...
pub trait ActorMessageClient: Send + Sync {
    ...
    /// Return the name of the the actor what belongs to this client
    fn name(&self) -> String;
}
    ...
    R: Send + Debug,
{
    ...
    async fn message(&self, m: ME) -> Res<()> {
        trace!(
            "`{}` actor client will send this message: `{:?}`",
            self.name(),
            &m
        );
        self.sender.send(Command::Message(m)).await.map_err(|e| {
            error!(
                "`{}` actor client unable to send this message: `{:?}`",
                self.name(),
                e
            );
