...
use crate::common::{
    BrokerMessages, BrokerRequests, BrokerResponses, ConsumerId, ConsumerMessages, ConsumerRef,
    JsonMessage, Topic,
};
    ...
    async fn push(&self, message: JsonMessage) {
        ...
        if let Some(clients) = clients {
            let futures: Vec<_> = clients
                ...
                .map(|(client, message)| {
                    client.message(ConsumerMessages::Publish(message.topic().clone(), message))
                })
                .collect();
...
/// Wrap `MessageBroker` into `HybridActor` which can accept messages and requests
